...
function buildRequiredText(match) {
  ...
  const bits = [];
  if (cleanText(msgs.required)) {
    bits.push(cleanText(msgs.required));
  }
  if (cleanText(msgs.partnership)) {
    bits.push(`Partnership: ${cleanText(msgs.partnership)}`);
  }
  if (cleanText(msgs.lastWicket)) {
    bits.push(`Last Wicket: ${cleanText(msgs.lastWicket)}`);
  }
  if (cleanText(match?.announcement1)) {
    bits.push(cleanText(match.announcement1));
  }
  return bits.join(" | ");
}
